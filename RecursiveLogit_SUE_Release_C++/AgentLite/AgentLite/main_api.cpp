...
#define MAX_NO_BISECTITERATION 1000
...
void g_ProgramStop()
{
	...
	cout << "RLSUELite Program stops. Press any key to terminate. Thanks!" << endl;
	getchar();
	exit(0);
};
...
public:
	...
	void Dial_subnetwork(float** adj_matrix) {
		...
		for (int od = 0; od < origin_node_vector.size(); od++) {
			...
			for (int i = 0; i < link_number_for_sub_network[od]; i++) {
				g_link_vector[sub_network_link_for_each_od[od][i]].flow_volume_per_period[0] += sub_network_volume_for_each_od[od][i];
			}

			//5. Update node numbers for each od
			...
			node_number_for_sub_network[od] = 0;
			for (int i = 0; i < m_g_number_of_nodes; i++) {
				if (node_SElist[i] != 0) {
					node_number_for_sub_network[od]++;
				}
			}
			sub_network_node_for_each_od[od] = new int[node_number_for_sub_network[od]];
			count = 0;
			for (int i = 0; i < m_g_number_of_nodes; i++) {
				if (node_SElist[i] != 0) {
					sub_network_node_for_each_od[od][count] = i;
					count++;
				}
			}
			delete[] node_SElist;
	...
	void RLSUE(Assignment& assignment, int iter_no) {
		...
		for (int od = 0; od < demand_volume_vector.size(); od++) {
			node_utility = new float[node_number_for_sub_network[od]];
			link_sys_utility = new float[link_number_for_sub_network[od]];
			link_con_prob = new double[link_number_for_sub_network[od]];
			link_prob = new double[link_number_for_sub_network[od]];
			link_flow = new double[link_number_for_sub_network[od]];
			link_label = new float[link_number_for_sub_network[od]];
			...
			Utility(od, origin_node, destination_node, network_cur, node_utility, link_sys_utility, \
				local_node_numbers, local_link_numbers);


			//link conditional probability
			vector<int> node_NOD_list;
			for (int i = 0; i < local_node_numbers; i++) {
				node_NOD_list.push_back(sub_network_node_for_each_od[od][i]);
			}
			...
			for (int i = 0; i < local_node_numbers; i++) {
				node_NoO_list.push_back(sub_network_node_for_each_od[od][i]);
			}
	...
	void Utility(int od, int origin_node, int destination_node, int* network_cur, \
		...
		int local_node_numbers, int local_link_numbers)
	{
		int destination_node_index = 0;
		for (int i = 0; i < local_node_numbers; i++) {
			if (sub_network_node_for_each_od[od][i] == destination_node)
		...
		for (int i = 0; i < local_node_numbers; i++) {
			node_record.push_back(sub_network_node_for_each_od[od][i]);
		}
		...
		while ((SElist.size() > 0)& (node_record.size() > 0)) {
			...
			for each (int pre_link in g_node_vector[node].m_incoming_link_seq_no_vector) {
					...
					for (int i = 0; i < local_link_numbers; i++) {
						if (network_cur[i] == next_link) {
							...
							for (int j = 0; j < local_node_numbers; j++) {
								if (sub_network_node_for_each_od[od][j] == g_link_vector[next_link].to_node_seq_no)
				...
				if (value == 1.0) {
					...
					for each (int next_link in g_node_vector[from_node].m_outgoing_link_seq_no_vector) {
						...
						for (int i = 0; i < local_link_numbers; i++) {
							if (network_cur[i] == next_link) {
								...
								for (int j = 0; j < local_node_numbers; j++) {
									if (sub_network_node_for_each_od[od][j] == from_node_to_node) {
					...
					for (int i = 0; i < local_node_numbers; i++) {
						if (sub_network_node_for_each_od[od][i] == g_link_vector[pre_link].from_node_seq_no)
							from_node_seq_no_index = i;
					}
					node_utility[from_node_seq_no_index] = mu * log(sum_exp);
					
					//************************************************************************************
					if (node_utility[from_node_seq_no_index] < -700) {
						node_utility[from_node_seq_no_index] = -700;
					}
					//************************************************************************************
		...
		for (int link = 0; link < local_link_numbers; link++) {
			...
			for (int j = 0; j < local_node_numbers; j++) {
				if (g_link_vector[network_cur[link]].to_node_seq_no == sub_network_node_for_each_od[od][j]) {
					to_node_seq_no_index = j;
				}
			}
			...
			//************************************************************************************
			if (link_sys_utility[link] < -700)
				link_sys_utility[link] = -700;
			//************************************************************************************
	...
	double  PerformBPR_2(double volume, int link_index, int* local_link_list)
	{
		volume = max(0, volume);  // take nonnegative values
		double capacity = g_link_vector[local_link_list[link_index]].link_spatial_capacity;
		double FFTT = g_link_vector[local_link_list[link_index]].free_flow_travel_time_in_min;
		double alpha = 0.15;
		double beta = 4;

		double VOC = volume / max(0.00001f, capacity);
		double avg_travel_time = FFTT + FFTT * alpha * pow(volume / max(0.00001f, capacity), beta);
		//double marginal_base = FFTT * alpha * beta * pow(volume / max(0.00001f, capacity), beta - 1);

		return avg_travel_time;
	}

	double OF_LinksDirectionalDerivative(double* MainVolume, double* SDVolume, \
		double Lambda, int no_links, int* local_link_list) {
		int k;
		double* Volume;
		double LinkCostSum = 0;

 		Volume = new double[no_links];

		for (k = 0; k < no_links; k++) {
			Volume[k] = MainVolume[k] + Lambda * SDVolume[k];
		}
		for (k = 0; k < no_links; k++) {
			LinkCostSum += PerformBPR_2(Volume[k], k, local_link_list) * SDVolume[k];
		}
	...
	double LinksSDLineSearch(double* MainVolume, double* SDVolume, int no_links, int* local_link_list) {
		...
		grad = OF_LinksDirectionalDerivative(MainVolume, SDVolume, 0.0, no_links, local_link_list);
		//cout << "Line search step" << "0.0" << endl;
		//cout << "Line search grad" << grad << endl;
		if (grad >= 0) {
			LastLambda = 0.0;
			return(0.0);
		}

		grad = OF_LinksDirectionalDerivative(MainVolume, SDVolume, 1.0, no_links, local_link_list);
		//cout << "Line search step" << "1.0" << endl;
		//cout << "Line search grad" << grad << endl;
		if (grad <= 0) {
			LastLambda = 1.0;
			return(1.0);
		...
		for (n = 1; n <= MinLineSearchIterations; n++) {
			grad = OF_LinksDirectionalDerivative(MainVolume, SDVolume, lambda, no_links, local_link_list);
			//cout << "Line search step" << lambda << endl;
			//cout << "Line search grad" << grad << endl;
	...
	void Update_link_flow_OD(Assignment& assignment, int iter_no) {
		for (int od = 0; od < demand_volume_vector.size(); od++) {
			...
			***********************Line search***********************
			double lambda_1;
			double lambda;
			double* SDVolume = new double[local_link_number];
			for (int i = 0; i < local_link_number; i++) {
				SDVolume[i] = link_flow_OD[od][i] - link_flow_aux[od][i];
			}
			lambda = LinksSDLineSearch(link_flow_OD[od], SDVolume, local_link_number, local_link_list);
			delete SDVolume;
			********************************************************
			*/

			for (int link = 0; link < local_link_number; link++) {
//				link_flow_OD[od][link] = (double)link_flow_OD[od][link] + (double)lambda * (double)(link_flow_aux[od][link] - link_flow_OD[od][link]);
	...
	void MAXUtility(Assignment& assignment, int iter_no)
	{
		for (int od = 0; od < demand_volume_vector.size(); od++) {
			...
			double* Pi = new double[local_node_number];
			...
			for each (int node in node_NoD_list){
				...
				if (iter_no == 1) {
					Pi_OD_aux[od] = new double[local_node_number];
				}
	...
	void Update_Pi_OD(Assignment& assignment, int iter_no) {
		for (int od = 0; od < demand_volume_vector.size(); od++) {
			int local_node_number = node_number_for_sub_network[od];
			if (iter_no == 1) {
	...
	void GAP(Assignment& assignment, double& gap_each_process) {
		gap_each_process = 0;
		for (int od = 0; od < demand_volume_vector.size(); od++) {
			int origin_node = origin_node_vector[od];
			int destination_node = destination_node_vector[od];
			...
			Utility(od, origin_node, destination_node, network_cur, node_utility, \
				link_sys_utility, local_node_numbers, local_link_numbers);

			for (int i = 0; i < local_link_numbers; i++) {
				if (link_flow_cur_od[i] > 10e-3) {
					int from_node = g_link_vector[network_cur[i]].from_node_seq_no;
					int to_node = g_link_vector[network_cur[i]].to_node_seq_no;
					...
					/*
					if (gap_abs > 100)
						cout << "GAP: link(" << g_node_vector[from_node].node_id << "-" << g_node_vector[to_node].node_id << \
						") at od (" << g_node_vector[origin_node].node_id << "->" << g_node_vector[destination_node].node_id << "):" << gap_abs << endl;
					*/
...
#pragma omp parallel for 
	...
	DeallocateDynamicArray(adj_matrix, assignment.g_number_of_nodes, assignment.g_number_of_nodes);
	...
	for (int iter_no = 1; iter_no <= iteration_number; iter_no++) {
		...
		//Step 3.1: Compute linkflow and Pi
#pragma omp parallel for 
		for (int ProcessID = 0; ProcessID < g_NetworkForSUE_vector.size(); ProcessID++) {
			g_NetworkForSUE_vector[ProcessID]->RLSUE(assignment, iter_no);
			g_NetworkForSUE_vector[ProcessID]->MAXUtility(assignment, iter_no);
			g_NetworkForSUE_vector[ProcessID]->Update_link_flow_OD(assignment, iter_no);
			g_NetworkForSUE_vector[ProcessID]->Update_Pi_OD(assignment, iter_no);
		}
...
#pragma omp parallel for 
		for (int ProcessID = 0; ProcessID < g_NetworkForSUE_vector.size(); ProcessID++) {
			g_NetworkForSUE_vector[ProcessID]->GAP(assignment, gap_for_each_od[ProcessID]);
		}

		/*
		for (int link = 0; link < assignment.g_number_of_links; link++) {
			cout << "link " << link << " volume:" << g_link_vector[link].flow_volume_per_period[0] << endl;
		}
		*/
